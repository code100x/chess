...
const Piece = React.memo(
  React.forwardRef<ChessPieceRef, PieceProps>(({ id, startPosition, square, size }, ref) => {
    const chess = useChessEngine();
    const refs = usePieceRefs();
    const pieceEnabled = useSharedValue(true);
    const { isPromoting } = useBoardPromotion();
    const { onSelectPiece, onMove, selectedSquare, turn } = useBoardOperations();
    ...
    const validateMove = useCallback(
      (from: Square, to: Square) => {
        return chess.moves({ verbose: true }).find((m) => m.from === from && m.to === to);
      },
      [chess]
    );
    ...
    const underlay = useAnimatedStyle(() => {
      ...
      return {
        ...
        transform: [{ translateX: translation.x - size / 2 }, { translateY: translation.y - size / 2 }],
      };
    }, [size]);
